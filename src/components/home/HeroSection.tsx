...
export default function HeroSection() {
  ...
  const badge = useSiteCopy("home-hero-badge", "Embodied Intelligence v2.0");
  const titlePrimary = useSiteCopy(
    "home-hero-title-primary",
    "AIeveR Robotics Limited",
  );
  const titleSecondary = useSiteCopy("home-hero-title-secondary", "雲芯機器人");
  ...
  const primaryCta = useSiteCopy("home-hero-primary-cta", "開始探索");
  const secondaryCta = useSiteCopy("home-hero-secondary-cta", "聯絡我們");
      ...
      <div className="relative z-10 mx-auto flex min-h-[clamp(640px,72vh,780px)] w-full max-w-[1440px] items-center px-6 py-24 md:px-10">
        <div className="max-w-[900px]">
          ...
          <motion.h1
            ...
            className="mt-7 text-[clamp(2.25rem,3.9vw,3.125rem)] font-medium leading-[1.56] tracking-normal text-text-primary"
          >
            <span className="block lg:whitespace-nowrap">{titlePrimary}</span>
            <span className="block whitespace-nowrap text-purple-light">
              {titleSecondary}
            </span>
          </motion.h1>
